...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
        ...
        self.add_flow3(datapath, 0, match, actions)
    ...
    def add_flow2(self, datapath, src, dst, src_ip, dst_ip, actions):
    ...
    def _packet_in_handler(self, ev):
        ...
        if pkt_igmp:
                ...
                for i in range (0,record_num):
                    ...
                    if mulcast_addr in self.group_mem: 
                        ...
                        for i in range(0,src_num):
                            temp.append(self.group_mem[mulcast_addr][i])
                        if src not in temp:
            ...
            print('multi_group_member')
            print(self.group_mem)    
        #else: 
        if pkt_ipv4:                 
            ...
            if dpid==1:
                ...
                if out_port != ofproto.OFPP_FLOOD:
                    match = parser.OFPMatch(eth_type=ether.ETH_TYPE_IP,in_port=in_port, ipv4_dst='224.0.0.88')
                ...
                if in_port==2:
                    ...
                    out_port2=2                
                if out_port1 :
                    action_1 = [parser.OFPActionSetQueue(0),parser.OFPActionOutput(out_port1),parser.OFPActionSetQueue(0),parser.OFPActionOutput(out_port2)]
                    match = parser.OFPMatch(eth_type=ether.ETH_TYPE_IP,in_port=in_port, ipv4_dst='224.0.0.88')
                    # verify if we have a valid buffer_id, if yes avoid to send both
                    # flow_mod & packet_out
                    if msg.buffer_id != ofproto.OFP_NO_BUFFER:
                        self.add_flow(datapath, 1, match, action_1, msg.buffer_id)                       
                        return
                    else:
